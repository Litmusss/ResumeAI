...
const EducationForm = ({ params }: { params: { id: string } }) => {
  ...
  const [aiGeneratedDescriptionList, setAiGeneratedDescriptionList] = useState<EducationSuggestion[]>([]);
  ...
  const generateEducationDescriptionFromAI = async (index: number) => {
    ...
    setIsAiLoading(true);
    setAiGeneratedDescriptionList([]); // Clear previous suggestions
  ...
  const selectDescription = (description: string) => {
    form.setValue(`education.${currentAiIndex}.description`, description, { 
      shouldValidate: true 
    });
    
    // Update the form context
    const updatedEducation = form.getValues("education");
    handleInputChange({
      target: {
        name: "education",
    ...
    <div>
      ...
      {aiGeneratedDescriptionList.length > 0 && (
        <div className="my-5" ref={listRef}>
          ...
          {aiGeneratedDescriptionList.map((item, index) => (
            <div
              key={index}
